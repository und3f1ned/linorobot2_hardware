...
static inline void set_microros_net_transports(IPAddress agent_ip,
                                               uint16_t agent_port) {
...
void moveBase() {
  // brake if there's no command received, or when it's only the first command
  // sent
...
void publishData() {
...
#ifdef BATTERY_DIP
  if (!skip_dip && battery_msg.voltage > 1.0 &&
      battery_msg.voltage < prev_voltage * BATTERY_DIP) {
    RCSOFTCHECK(rcl_publish(&battery_publisher, &battery_msg, NULL));
    syslog(LOG_WARNING, "%s voltage dip %.2f", __FUNCTION__,
           battery_msg.voltage);
    skip_dip = 5;
  }
  if (skip_dip)
  ...
  battery_msg.voltage = prev_voltage =
      battery_msg.voltage * 0.01 + prev_voltage * 0.99;
